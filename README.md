...
用户添加URL转发记录时，DNS会自动为用户添加CNAME记录，

而CNAME记录指向的地址则是提供的URL转发服务器地址，目的就是通过URL转发服务器来帮助用户实现转发功能。

当访问者访问转发前域名时，实际上是提供的URL转发服务器发起请求，

如果是隐性URL转发，则是通过DNS的应用来嵌套用户目标的URL，并返回带有iframe的页面给到访问者；

如果是显性URL转发，则是通过301或302重定向跳转到转发后页面。
