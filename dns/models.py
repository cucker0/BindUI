...
class Record(BasicModel):
    ...
    serial = models.BigIntegerField('serial', null=True, blank=True, default=None, help_text='serial for SOA record,Zone序列号,范围[0, 4294967295]')
    ...
    basic = models.IntegerField('basic', default=0, help_text='是否为基础记录，记录是否允许重复。0:可重复非基础记录, 1:可重复基础记录， 2:不可重复基础记录，3:被显性URL或隐性URL关联的记录 ，200:隐性URL转发，301:显性URL 301重定向，302:显性URL 302重定向。'
                                                              '基础记录是指SOA、NS记录，非基础记录是指除SOA、NS之外的记录；重复部分是指host、type、zone_id字段的组合，下面的与此相同')
    associate_rr_id = models.BigIntegerField('associate_rr_id', null=True, blank=True, default=None, help_text='关联的 Resource Record ID，用于显性URL、隐性URL')
